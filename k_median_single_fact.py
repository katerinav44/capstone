...
def k_median_single_factory(bays, facts, n_vehicles, k):
    ...
    problem += (
        ...
        # using run factory - this only works for single factory
        # sum(d[j] * Y[j] for j in range(n_facts))

        # (Attempt 2 - run_factory k > 1) sum(run_factory(facts_list[j], [bays_list[i] for i in range(n_bays) if X[i][j] == 1], 1, True) * Y[j] for j in range(n_facts)) 
        # (Attempt 3) z
    ), "k_median"
    ...
    for j in range(n_facts):
        # (Attempt 3) problem += run_factory(facts_list[j], [bays_list[i] for i in range(n_bays) if X[i][j] == 1], 1, True) * Y[j] <= z
