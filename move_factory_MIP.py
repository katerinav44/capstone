...
def k_median_single_factory_moving(bays, facts, n_vehicles):
    ...
    Z = pl.LpVariable.dicts("start_factory", indices=range(n_facts), lowBound=0, upBound=1, cat='Binary')  # Starting factory indicator
    ...
    # Ensure exactly one factory is chosen as the starting location
    problem += sum(Z[j] for j in range(n_facts)) == 1

    # If a factory is selected as the starting location, it does not incur movement penalty
    for j in range(n_facts):
        problem += Y[j] >= Z[j]  # If factory j is starting, it cannot be moved to
    ...
    for i in range(n_facts):
        if i==0:
            #starting location
            t=run_factory(facts_L[i], factory_assignments[facts_L[i]], n_vehicles, True)
            total_time+=t
            movement_times.append(t)
        elif len(factory_assignments[facts_L[i]])>0:#make sure bays were assigned to this factory
            #movement location
            t=movement_penalty+run_factory(facts_L[i], factory_assignments[facts_L[i]], n_vehicles, True)
            total_time+=t
            movement_times.append(total_time)
    #last element of movement_times is total construction time

    return factory_assignments, movement_times
...
if __name__ == "__main__":
    ...
    #bays = [(1,1), (999999, 999999)]
    #facts = [(0, 0), (1000000, 1000000), (500,500)]
    factory_assignments, movement_times=k_median_single_factory_moving(bays, facts, n_vehicles)
    ...
    for factory, assigned_bays in factory_assignments.items():
        if assigned_bays != []: #no bays assigned to factory
            ...
            plt.scatter(*zip(*assigned_bays), marker='.', color=color, label='bays assigned to factory '+str(factory)+' at time t='+str(movement_times[i]))
    plt.plot(*zip(*bbox), color='blue', label='bounding box for test')
    plt.xlabel('x')
    plt.ylabel('y')
    plt.legend()
    ttm=movement_times[-1]
